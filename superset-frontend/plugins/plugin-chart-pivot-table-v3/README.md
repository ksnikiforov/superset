...
- Starts collapsed by default; each expansion issues a scoped `/api/v1/chart/data` call and shows a spinner on the header cell.
...
Key form fields: `groupbyRows`, `groupbyColumns`, `metrics`, `aggregateFunction`, `startCollapsed`, `initialDepth`, `maxDepthPerFetch`, `rowTotals`, `colTotals`, `rowSubTotals`, `colSubTotals`, `rowOrder`, `colOrder`, `valueFormat`, `dateFormat`, `currencyFormat`, `allowRenderHtml`, `metricsLayout`, `transposePivot`, `combineMetric`.
