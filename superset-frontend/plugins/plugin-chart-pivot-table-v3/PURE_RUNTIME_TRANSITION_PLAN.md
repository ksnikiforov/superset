...
`8e9db3b4bb refactor(pivot-table-v3): batch expansion state commits`:
...
- Production `src`: `12125` insertions, `11639` deletions, net `+486`.
- Current production `src` TypeScript/TSX total: `33996` lines.
...
| Gate 4: one tree materializer             |        90% | `materializePivotTree.ts` owns fact-to-tree materialization, Values/metric/measure axes, subtotal leaf injection, and measure-leaf value application. Export no longer repairs row depth semantics, infers visible row hierarchy depth from cloned DOM rows, or reconstructs visible row paths by scanning sibling DOM rows, but still needs a cleaner workbook model boundary.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- Export still clones and reshapes the rendered DOM table for workbook output.
  ...
  inference, and sibling-row hierarchy reconstruction are gone, but export is
  not yet a sibling workbook model derived directly from the same tree/program
  boundary as render.
