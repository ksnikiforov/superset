...
As of May 14, 2026, after the Gate 7 shared view-props checkpoint:

- Gate-weighted architecture estimate: **97%**.
- Delivery remaining estimate: **2-6%**, mostly final cleanup, validation,
...
- Production `src`: `13878` insertions, `12661` deletions, net `+1217`.
- Current production `src` TypeScript/TSX total: `34727` lines.
...
- Source-only baseline comparison from
  `7088db374448845ef6e71cf74817aa53efbc5fc1`: `13878` insertions, `12661`
  deletions, net `+1217`; current production `src` TypeScript/TSX total is
  `34727` lines.
...
- Chart component cleanup: not complete; runtime-layout state, prop sync,
  ...
  live outside the chart. The chart still owns top-level view composition and
  interaction panel assembly. Column-sort state and callbacks now live with the
  render model.
