...
Latest program-shape cleanup: `PivotProgram.rows` and `PivotProgram.columns`
were deleted. Axis programs are now derived from dimensions, metrics,
...
sibling expansion targets; once the batch is built, the request shape contains
only axis, parent path, sibling values, and target coverage.

Latest query-spec cleanup: batch query specs no longer expose unused
...
Latest expansion scheduler cleanup: the separate opposite-root suppression
...
Latest expansion warning cleanup: expansion warnings no longer maintain a
parallel ref-backed warning map. The hook deduplicates warnings through the
React state updater, leaving one warning state source.
...
Latest query-spec cleanup: root/bootstrap specs no longer construct a fake
...
fixtures now convert rendered metric/subtotal paths back to the same dimensional
query scopes used by the runtime, so ancestor/subtotal regressions are caught at
fact ingestion and materialization boundaries instead of patched by render-time
fallbacks.

Latest seamless-ingestion cleanup: seamless runtime updates now fetch planned
query specs directly into a `PivotFactStore`, matching the expansion fetch
boundary. The duplicate async initial-runtime wrapper is gone; chunked ingestion
is owned by the planned-spec/fact-store ingestion path and materialization reads
the loaded store.

Latest expansion-ingestion cleanup: expansion fetch now passes request
...
Latest query-boundary cleanup: `query/normalizeExtraFormData.ts` is gone.
...
Latest render-policy cleanup: hidden single-metric header visibility is no
longer carried as row/column state through `PivotLayoutResult`. The rule derives
...
Latest control cleanup: metric rename repair now moves only the canonical
metric key and rewrites references that resolve to that same key. The old
control-side identifier set that treated labels, verbose names, metric names,
and formatting aliases as interchangeable is removed.
...
Latest render-boundary cleanup: `usePivotLayout` no longer exports a
`buildRenderModelConfig` adapter or owns render child callbacks. Layout now
returns compiled layout plus the small metric/subtotal policy needed by
formatting/rendering, while `usePivotRenderModel` builds the render model
configuration at the render boundary. Query-spec fetch context also stopped
...
Branch requests no longer carry duplicate top-level `axis`/`path` data, batch
requests derive query coverage from their target group, and intersection
requests carry one compiled target object instead of parallel row/column/depth
...
Latest hydration planner cleanup: `planHydrationIteration` no longer returns
visible depth metadata to callers, and the single-use cross-axis intersection
...
Latest expansion target cleanup: compiled expansion coverage targets no longer
carry top-level visible row/column depth fields. Visible depth stays local to
...
Latest branch-coverage cleanup: expansion support coverage now consumes the
compiled coverage target directly. Branch fact coverage no longer receives
duplicated row/column depths or dimension arrays beside the manifest target it
is already planning from.

Latest intersection-target cleanup: intersection fetch targets no longer carry
parallel row/column path-key arrays. Query filters, fact-store scope, and
...
Latest layout-result authority cleanup: `PivotLayoutResult` no longer exposes
`measureHierarchy` or `axisCoverageNeeds` aliases. Consumers read those facts
