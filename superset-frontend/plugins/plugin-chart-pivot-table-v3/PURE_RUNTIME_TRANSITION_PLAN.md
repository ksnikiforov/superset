...
- Production `src`: `7897` insertions, `9109` deletions, net `-1212`.
- Current production `src` TypeScript/TSX total: `32298` lines.
...
(`2536` lines), `useExpansionEngine.ts` (`1687` lines),
...
| Gate 2: query planning from program/coverage |        89% | Initial/root/branch/batch query paths use coverage metadata and canonical paths. Expansion planning now trusts fetched coverage depth directly, and no-dimension runtime layouts now require explicit root coverage instead of treating empty shape as implicitly loaded. Remaining complexity is mostly support/totals coverage composition.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Gate 4: one tree materializer                |        87% | `materializePivotTree.ts` owns fact-to-tree materialization, Values/metric/measure axes, subtotal leaf injection, row subtotal labeling, and measure-leaf value application consumed by the chart. The last initial-tree compatibility wrapper is deleted, raw-record tree construction has moved out of production `src` into test fixtures, and test-only metric-axis fixture wrappers no longer live in production runtime exports. Remaining work is export parity and materializer module split only where it deletes callers.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Gate 5: expansion reducer/runtime effects    |        77% | Expansion has stronger pure helpers, typed coverage, shared latest-request lifecycles, reducer-owned pending/loading state, axis-neutral pending/expanded state setters, shared local/single/batch fetch execution, one fetch-result shape, shared request-id construction, shared fetched-result delta application, pure reinitialized expansion-state resolution, coverage-only planner satisfaction, and visible-key persistence now reuses the already-built render model. Auto-expansion seeding now belongs to the state model, the unused legacy fetch-coordinator prototype is gone, the single-use staging-tree helper has been folded into hydration, and stable layout trims no longer project cell values, fetched coverage, existing deep tree shape, or locally trimmed/promoted tree nodes across semantic layout changes. The hook still owns hydration iteration, cancellation, and React state commits.                                                                                                                                                                                                                                                                           |
...
| Gate 7: chart component cleanup              |        50% | `PivotTableChart.tsx` now delegates seamless fetch planning/materialization and runtime-layout comparison to layout/runtime helpers and no longer re-materializes measure leaves, vetoes upstream trees based on leaf source metrics, rejects incoming props trees through the stale-coverage regression branch, checks rendered tree shape for recovery coverage, uses tree projection to decide layout-change fetches, calls a single-use committed-tree sync guard, carries its own runtime-layout normalizer, computes structural and coverage layout-fetch decisions separately, freezes the whole view-prop bundle during seamless updates, or explodes formatting state into individual view props. The old committed-tree sync guard and layout fetch-policy modules are gone; exact runtime-layout fact coverage and layout-fetch policy now live in the runtime coverage boundary, no-dimension layouts fetch/materialize exact root coverage, seamless pending display now snapshots only render data, and `PivotTableView` receives formatting as one render concern. It still owns committed-tree sync, dimension filters, interaction wiring, and several local UI-state controllers. |
...
- Production line-deletion completion: **87%**. Pre-existing files are net `5622`
  lines smaller, and total production source is now net `1212` lines smaller
...
- Interactivity requirement completion: **71%**. Layout refresh and expansion
  ...
  shared render traversal now avoids repeated full-node child scans. The plugin
  still needs lower-cost large-result commits and less chart-owned
  fallback/loading orchestration.
- Query visibility requirement completion: **89%**. Planning is now strongly
  coverage based and no longer treats rendered child/cell shape as fetched
  coverage. Empty row/column layouts also require exact root fact coverage
  before grand-total materialization. Remaining risk is support/totals coverage
  composition and hydration edge paths still tied to hook compatibility state.
- Simplification/deletion requirement completion: **84%**. Recent Gate 4/Gate 6
  ...
  module. The stable-trim cleanup also removed local cell/fetched-coverage
  projection and local tree-shape repair across semantic trims, but deeper row
  subtotal policy, hydration/persistence, and chart controller logic still keep
  meaningful old complexity alive.
...
- Remaining pipeline debt is not a missing layer; it is old orchestration still
  ...
  `PivotTableChart.tsx` committed-tree sync, `useExpansionEngine.ts`
...
- Deletion payoff: **past break-even and improving**. Pre-existing source files
  are net `-5622`, and added runtime/helper files now leave the plugin net
  `-1212`.
- Interactivity: **partially improved**. Latest-only requests, reducer loading
  ...
  materialization/render commits is still not solved.
...
- The recent deletion-positive Gate 4/Gate 6 cleanup removed rendered-tree
  ...
  latest chart/view/expansion cleanup slices have pushed the production tree
  net-negative again.
...
4. Keep every deletion-oriented slice net-negative in production `src`. A
   performance slice must name the line-count tradeoff and should be paired with
   deletion soon after; test-line deletion remains a non-goal.
...
- `7897` insertions
- `9109` deletions
- net `-1212` production source lines
- current `src` code total: `32298` lines
...
- Modified files: `+3487 / -7690`, net `-4203`
...
| `PivotTableChart` |       376 |       990 |  `-614` | Chart shrinkage is now visible, runtime-layout fetch decisions are delegated to the runtime coverage boundary, and seamless pending display snapshots only render data instead of the whole view prop bundle. |
...
- Gate 5 staging-helper cleanup: the single-use `stagingTree.ts` helper and its
  module-only tests were deleted. Hydration now keeps its staged branch deltas
  local to the hydration loop, and the dead `buildBootstrapPlan` wrapper was
  removed from the bootstrap planner.
- Additional production change for that follow-up: `+19 / -74`, net `-55`.
- Verification after the staging-helper cleanup: single-process Jest passed for
  prefetch batching, satisfied-target prefetch, cross-axis expansion, expansion
  planner, runtime-coverage query specs, and build-query suites (`25` tests).
- Gate 6 render traversal performance follow-up: `findChildren` now uses a
  child lookup cached by immutable node-map identity, and `buildVisibleList`
  no longer carries an unused default child resolver. This is not a deletion
  slice; it trades `+29 / -8`, net `+21` production source lines for avoiding
  repeated full-node scans during render/visibility traversal.
- Verification after the child lookup cache: single-process Jest passed for
  render model, expansion engine, chart basic regression smoke, and interaction
  layout suites (`36` tests), and touched-file ESLint passed.
- Runtime coverage API cleanup: the unused `buildExpansionFactCoverage` helper,
  its input type, and its private depth-counting helpers were deleted. The
  active expansion/query paths already use branch coverage and Values-level
  checks instead.
- Additional production change for that follow-up: `+0 / -74`, net `-74`.
- Verification after the coverage API cleanup: single-process Jest passed for
  runtime coverage, runtime-coverage query specs, fetch-branch batching, and
  expansion planner suites (`26` tests), and touched-file ESLint passed.
- Runtime materializer finalization cleanup: sync and async initial
  materialization now share the Grand total relabeling plus measure-leaf value
  application instead of carrying duplicate post-merge blocks.
- Additional production change for that follow-up: `+27 / -32`, net `-5`.
- Verification after the materializer finalization cleanup: single-process Jest
  passed for runtime ingestion/materialization and basic chart regression smoke
  suites (`11` tests), and touched-file ESLint passed.
- Gate 6 column-display module cleanup: the single-use `columnDisplay.ts`
  module was folded into `usePivotRenderModel.ts`, and the redundant grand-total
  column label branch was collapsed while preserving metric-first/subtotal
  header behavior.
- Additional production change for that follow-up: `+107 / -138`, net `-31`.
- Verification after the column-display module cleanup: single-process Jest
  passed for render model, column totals, interaction layout, and metric-first
  column-metrics suites (`65` tests), and touched-file ESLint passed.
- Gate 5 expansion fetch-result cleanup: expansion fetch execution now returns
  a single `targets` result shape and builds single/batch request group IDs
  inside `fetchExpansionTargets`, removing duplicate caller callbacks while
  keeping local/single/batch execution shared.
- Additional production change for that follow-up: `+73 / -95`, net `-22`.
- Verification after the expansion fetch-result cleanup: single-process Jest
  passed for expansion engine, cross-axis no-blanks, cross-axis concurrent, and
  initial-depth prefetch suites (`9` tests), and touched-file ESLint passed.
- Gate 7 formatting/view prop cleanup: `PivotTableChart.tsx` now passes the
  formatting hook result to `PivotTableView` as one formatting concern instead
  of expanding a dozen formatting props, and the view owns the internal
  destructuring.
- Additional production change for that follow-up: `+20 / -62`, net `-42`.
- Verification after the formatting/view prop cleanup: single-process Jest
  passed for PivotTableView, basic chart regression smoke, and chart metrics
  suites (`35` tests), and touched-file ESLint passed from `superset-frontend`.
- Gate 7 formatting/databar setup cleanup: databar scale targets are now built
  in one memoized set, redundant post-empty-check databar guards are gone, and
  the non-waterfall scale map reuses the computed scale map.
- Additional production change for that follow-up: `+80 / -100`, net `-20`.
...
- Verification after the chart alias cleanup: single-process Jest passed for
  ...
  `superset-frontend`.
- Gate 7 dimension formatting runtime-map cleanup: row and column dimension
  formatting now each build key lookups and compiled Excel formula lookups in a
  single pass instead of maintaining separate key-map and Excel-map builders.
- Additional production change for that follow-up: `+43 / -67`, net `-24`.
- Verification after the dimension formatting cleanup: single-process Jest
  passed for PivotTableView, basic chart regression smoke, and chart metrics
  suites (`35` tests), and touched-file ESLint passed from `superset-frontend`.
- Gate 7 metric formatting runtime-map cleanup: metric formatting now builds
  formatting-key lookups and compiled Excel formula lookups in one pass,
  matching the dimension formatting runtime-map shape.
- Additional production change for that follow-up: `+42 / -49`, net `-7`.
- Verification after the metric formatting cleanup: single-process Jest passed
  for PivotTableView, basic chart regression smoke, and chart metrics suites
  (`35` tests), and touched-file ESLint passed from `superset-frontend`.
- Gate 7 strip drop-target cleanup: row and column interaction chip strips now
  share one typed drop-target hook instead of carrying duplicate strip-level
  dimension/value drop handlers in `PivotTableChart.tsx`.
- Additional production change for that follow-up: `+50 / -58`, net `-8`.
- Verification after the strip drop-target cleanup: single-process Jest passed
...
- Verification after the local layout-projection cleanup: single-process Jest
  ...
  `superset-frontend`.
- Gate 2/Gate 7 runtime fetch-decision cleanup: the standalone layout
  fetch-policy module is gone. Runtime coverage now owns runtime-layout
  equality/order checks and the combined "should fetch" decision, while
  `PivotTableChart.tsx` asks one coverage-boundary question instead of
  separately computing structural and coverage fetch needs.
- Additional production change for that follow-up: `+202 / -206`, net `-4`.
- Verification after the runtime fetch-decision cleanup: single-process Jest
  passed for layout fetch policy, runtime coverage, interaction layout, and
  seamless interaction regressions (`76` selected tests), and touched-file
  ESLint passed from `superset-frontend`.
- Gate 5 no-local-aggregate-projection cleanup: layout trim transitions no
  longer roll child cell values up to surviving parent nodes, and fetched
  coverage is no longer remapped across stable semantic trims. Removing a
  trailing row/column dimension now fetches exact parent aggregate coverage
  instead of treating previously expanded children as a valid substitute.
- Additional production change for that follow-up: `+5 / -338`, net `-333`.
- Verification after the no-local-aggregate-projection cleanup:
  single-process Jest passed for layout-transition, fetched-requests, and the
...
- Gate 4 materialization-boundary cleanup: measure-leaf value application moved
  from `measureLeaves.ts` into `runtime/materializePivotTree.ts`. Production
  measure-leaf helpers now stop at leaf IDs, output keys, offsets, and scalar
  value computation; tree mutation remains inside the materializer boundary.
- Additional production change for that follow-up: `+66 / -66`, net `0`.
- Verification after the materialization-boundary cleanup: single-process Jest
  passed for the touched sorting, metrics, interaction-layout,
  measure-leaf-tier, row totals, metric-tier, and expansion measure-leaf suites
  (`118` tests); touched-file ESLint and `git diff --check` passed from
  `superset-frontend`.
- Gate 5 no-local-tree-projection cleanup: layout transitions no longer trim
  removed-depth nodes or promote parent nodes when a semantic layout change
  arrives without fresh data. The helper now keeps the current tree only as a
  display snapshot, caps stale auto-expansion to the last stable expandable
  prefix, and leaves tree shape changes to fresh materialization. Runtime
  coverage also requires explicit root coverage for no-dimension layouts, so
  grand-total-only transitions fetch/materialize exact facts instead of using
  stale row or column shape.
- Additional production change for that follow-up: `+5 / -211`, net `-206`.
- Verification after the no-local-tree-projection cleanup: single-process Jest
  passed for layout-transition, seamless interaction, and runtime coverage
  suites (`47` tests), and touched-file ESLint passed from
  `superset-frontend`.
- Gate 7 seamless display-snapshot cleanup: pending seamless layout refreshes no
  longer freeze the whole `PivotTableView` prop bundle. `PivotTableChart.tsx`
  now snapshots only the render model, render tree, and expanded row/column sets
  while the committed runtime update settles, leaving callbacks, formatting,
  warnings, sizing, and other view concerns live.
- Additional production change for that follow-up: `+32 / -35`, net `-3`.
- Verification after the seamless display-snapshot cleanup: single-process Jest
  passed for full seamless expansion, interaction layout, and interaction filter
  seamless suites (`58` tests), and touched-file ESLint passed from
  `superset-frontend`.
...
- Gate 2 is largely complete (`89%`). Initial, root, branch, and batch query paths now
  ...
  fetched-depth shape compatibility, implicit root-layout coverage, and
  chart-owned layout fetch branching have been removed from the main path.
...
- Gate 4 is in progress (`87%`). `src/pivot/runtime/materializePivotTree.ts` now owns
  ...
  longer performs a second measure-leaf materialization pass, and
  `measureLeaves.ts` no longer mutates trees; runtime/upstream materialization
  owns that contract.
- Gate 5 is started but still the largest source of complexity (`77%`).
  ...
  local/single/batch helper path, fetch results use one target-list shape,
  request-group ID construction is local to that helper path, fetched-result
  delta application is shared, reinitialized expansion-state resolution is pure,
  visible-key persistence reuses the render model, fetched coverage now
  ...
  values, fetched coverage, existing deep tree shape, or local tree
  trim/promotion up to surviving parent nodes.
...
- Gate 6 is partially complete (`84%`). Projection now drives toggle eligibility,
  ...
  folded into the render boundary, repeated child lookup scans are cached per
  immutable node map, one row-end Values child-filter branch has been deleted,
  and the row/column child-filter prelude is now shared.
  Remaining work is mostly deeper row subtotal policy.
- Gate 7 has only been lightly reduced (`50%`). `PivotTableChart.tsx` is smaller than
  baseline, and the view formatting prop surface is narrower, but at roughly
  `2536` lines the chart still owns committed-tree sync, dimension filters,
  interaction wiring, and controller-like responsibilities.

Current largest production hotspots by line count:

- `PivotTableChart.tsx`: `2536` lines. It still owns committed-tree sync,
...
- `materializePivotTree.ts`: `1326` lines. It is now production-only runtime
  materialization surface, but still large enough that splits must be tied to
  caller deletion or export parity.
- `PivotInteractionPanel.tsx`: `1186` lines. It is large, but less tied to the
...
- `usePivotRenderModel.ts`: `801` lines. This is the next likely Gate 4/Gate 6
  deletion surface because render repair and folded column-display policy should
  shrink now that materialization creates metric, measure, and subtotal nodes.
