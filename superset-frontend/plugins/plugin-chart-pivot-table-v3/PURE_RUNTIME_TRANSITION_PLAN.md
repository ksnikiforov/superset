...
| Metric          | Value           | Note                                   |
| --------------- | --------------- | -------------------------------------- |
| Active tasks    | 6               | 5 in progress, 1 not done              |
| Completed tasks | 11              | Crossed out in the progress table      |
| Plugin tests    | Green           | 90 suites, 704 tests passed            |
| Next best move  | Split subtotals | Focus `materializePivotTree` authority |

## Current Metrics

Baseline: `7088db374448845ef6e71cf74817aa53efbc5fc1`

Latest slice delta: measured from `a3a3f24627d97f37d7a3c85c1529fe4d77ef35dc`, **+26** production `src` lines (`+73` / `-47`). Scheduler authority moved out of the hook; the next runtime slice should pay down the executor helper surface.

| Scope                         | Baseline Lines | Current Lines |  Delta | Target          |
| ----------------------------- | -------------: | ------------: | -----: | --------------- |
| Full production `src`         |         33,510 |        26,547 | -6,963 | <20,000         |
| Strict core pipeline          |         12,907 |        10,583 | -2,324 | <8,000          |
...
| ------------------------------------------------------ | -------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------- | --------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Full production `src` size                             | Above target   | Source is smaller than baseline but still above the transition target.                                          | 26,547 lines                | <20,000 lines   | Final transition state is below target without moving production authority into test fixtures or compatibility adapters.                     |
| Strict core pipeline size                              | Above target   | Core still contains too much scheduler/materializer/render-policy code.                                         | 10,583 lines                | <8,000 lines    | Runtime, expansion, query, layout/core, domain helper, and support scope total is below target.                                              |
