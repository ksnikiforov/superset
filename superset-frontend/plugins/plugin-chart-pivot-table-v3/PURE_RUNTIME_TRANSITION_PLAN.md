...
| Metric          | Value            | Note                                  |
| --------------- | ---------------- | ------------------------------------- |
| Active tasks    | 7                | 6 in progress, 1 not done             |
| Completed tasks | 10               | Crossed out in the progress table     |
| Plugin tests    | Green            | 90 suites, 704 tests passed           |
| Next best move  | Reduce scheduler | Continue `useExpansionEngine` cleanup |

## Current Metrics

Baseline: `7088db374448845ef6e71cf74817aa53efbc5fc1`

Latest slice delta: measured from `cc23d0ebcbc7fa79cd318c0672d634a5e3862c75`, **-2** production `src` lines (`+184` / `-186`). Scheduler authority moved out of the hook, and the coupled paydown kept the slice deletion-positive.

| Scope                         | Baseline Lines | Current Lines |  Delta | Target          |
| ----------------------------- | -------------: | ------------: | -----: | --------------- |
| Full production `src`         |         33,510 |        26,521 | -6,989 | <20,000         |
| Strict core pipeline          |         12,907 |        10,557 | -2,350 | <8,000          |
| Core pipeline dirs diagnostic |          8,698 |         7,653 | -1,045 | Lower over time |
...
| Reduce expansion scheduler state in `useExpansionEngine.ts`          | In progress | Reinitialization decisions and axis-wide expanded-state rebuilds moved to transition helpers; hook still owns commit, persistence, and request triggering. |      9 | Medium      |          8 |    5 | 360 | `useExpansionEngine.ts` no longer owns separate request lifecycle, loading-key derivation, and hydration execution branches; those live in manifest/transition helpers with tests, and the coupled slice is deletion-positive.        |
...
| ------------------------------------------------------ | -------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------- | --------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| Full production `src` size                             | Above target   | Source is smaller than baseline but still above the transition target.                                          | 26,521 lines                | <20,000 lines   | Final transition state is below target without moving production authority into test fixtures or compatibility adapters.                     |
| Strict core pipeline size                              | Above target   | Core still contains too much scheduler/materializer/render-policy code.                                         | 10,557 lines                | <8,000 lines    | Runtime, expansion, query, layout/core, domain helper, and support scope total is below target.                                              |
| Diagnostic core dirs size                              | Watch          | Core pipeline dirs are below baseline but still useful as a drift signal.                                       | 7,653 lines                 | Lower over time | `pivot/runtime`, `pivot/expansion`, `pivot/query`, `pivot/layout`, and `pivot/core` do not grow without deleting a larger authority surface. |
| Plugin test suite                                      | Guarded        | Plugin tests pass after scheduler helper extraction.                                                            | 90 suites, 704 tests passed | 0 failing tests | `npm test plugins/plugin-chart-pivot-table-v3` passes.                                                                                       |
