...
As of May 14, 2026, after the Gate 1 runtime-placement checkpoint:

- Gate-weighted architecture estimate: **97%**.
- Delivery remaining estimate: **9-17%**, mostly final cleanup, validation,
...
- Production `src`: `13306` insertions, `12415` deletions, net `+891`.
- Current production `src` TypeScript/TSX total: `34193` lines.
...
- Source-only baseline comparison from
  `7088db374448845ef6e71cf74817aa53efbc5fc1`: `13306` insertions, `12415`
  deletions, net `+891`; current production `src` TypeScript/TSX total is
  `34401` lines.
...
- Chart component cleanup: not complete; runtime-layout state, prop sync, and
  runtime persistence execution plus persisted-filter sync now live outside
  `PivotTableChart.tsx`, but the chart still owns committed tree/fact state,
  interaction callback wiring, and several controller-like effects.
