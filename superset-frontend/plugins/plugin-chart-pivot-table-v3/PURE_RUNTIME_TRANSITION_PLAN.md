...
- Production `src`: `13282` insertions, `12406` deletions, net `+876`.
...
- Source-only baseline comparison from
  `7088db374448845ef6e71cf74817aa53efbc5fc1`: `13282` insertions, `12406`
  deletions, net `+876`; current production `src` TypeScript/TSX total is
  `34386` lines.
...
- Chart component cleanup: not complete; runtime-layout state, prop sync, and
  runtime persistence execution now live outside `PivotTableChart.tsx`, but the
  chart still owns committed tree/fact state, interaction callback wiring, and
