...
- Verification after the seamless display-snapshot cleanup: single-process Jest
  ...
  `superset-frontend`.
- Gate 7 stale-recovery fetch-decision cleanup: stale dashboard remount
  recovery no longer imports or caches the exact coverage predicate in
  `PivotTableChart.tsx`. The recovery effect now reuses the runtime
  `shouldFetchRuntimeLayout` decision with the committed layout on both sides,
  so the chart does not own a separate runtime coverage verdict.
- Additional production change for that follow-up: `+11 / -6`, net `+5`.
- Verification after the stale-recovery fetch-decision cleanup: single-process
...
- Gate 7 has only been lightly reduced (`50%`). `PivotTableChart.tsx` is smaller than
  ...
  `2541` lines the chart still owns committed-tree sync, dimension filters,
