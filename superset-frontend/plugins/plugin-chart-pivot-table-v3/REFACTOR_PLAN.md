...
1) Every query generated by pivot v3 MUST include a `query_name` with the prefix `pivot_v3|row{n}|col{m}` (see `formatQueryName()` in `src/pivot/engine/query/queryName.ts`).
...
- initial and update planning MUST share the same batch-optimization step:
 ...
 - move the existing batching primitives into `pivot/query` as pure code:
   - `buildBatchSignature()` (currently `pivot/engine/query/batchSignature.ts`)
   - `optimizeFetchPlan()` / `MAX_BATCH_SIBLINGS` (currently `pivot/engine/query/fetchPlanOptimizer.ts`)
